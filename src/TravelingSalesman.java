...
public class TravelingSalesman {
	...
	private static int computeTSP(int[][] distanceMatrix){
		...
		for (int i = 0; i < n; i++)
			dp[i][startVertex] = distanceMatrix[i][startVertex];
		...
		try {
			...
			while ((line = inReader.readLine()) != null){   
			    ...
			    for (int col = 0; col < arr.length; col++) 
			        distanceMatrix[row][col] = Integer.parseInt(arr[col]);
			    
			    row++;              
			}     

			inReader.close();
			computeTSP(distanceMatrix);			
